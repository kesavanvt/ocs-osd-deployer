...
const (
	...
	pagerdutySecretName          = "managed-ocs-pagerduty"
...
func (r *ManagedOCSReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	pagerdutySecretPredicates := builder.WithPredicates(
		predicate.NewPredicateFuncs(
			func(meta metav1.Object, _ runtime.Object) bool {
				return meta.GetName() == pagerdutySecretName
			},
		),
	)
...
func (r *ManagedOCSReconciler) initReconciler(req ctrl.Request) {
	...
	r.pagerdutySecret.Name = pagerdutySecretName
...
func (r *ManagedOCSReconciler) reconcileAlertmanagerConfigSecret() error {
	...
	_, err := ctrl.CreateOrUpdate(r.ctx, r.Client, r.alertmanagerConfigSecret, func() error {
		...
		if len(pagerdutySecretData["PAGERDUTY_KEY"]) != 0 {
			pagerdutyServiceKey = string(pagerdutySecretData["PAGERDUTY_KEY"])
		}
		if len(pagerdutySecretData["serviceKey"]) != 0 {
			pagerdutyServiceKey = string(pagerdutySecretData["serviceKey"])
		}
		if pagerdutyServiceKey == "" {
			return fmt.Errorf("Pagerduty service key is not defined")
		}
		...
		if err != nil {
			return fmt.Errorf("Unable to parse alertmanager conifg: %v", err)
		}
